              ...
              if IsFileExist(src) then
                if _bCreateMapFileTree ~= nil then
                  if _bReCreateMapFileTree == "Y" then
                    ...
                    pv("tmpMFT = ["..tmpMFT.."]")
                    os.execute([[START /B /wait "" /MIN cmd /c Del /f /q /s "]]..tmpMFT..[[.txt" 1>NUL 2>NUL]])
                    os.execute([[START /B /wait "" /MIN cmd /c Del /f /q /s "]]..tmpMFT..[[.lua" 1>NUL 2>NUL]]) --was /wait "" /B /MIN
                  end
...
function GetPrecKeyWordsInfo(prec_key_words)
  ...
  return [[{]]..string.sub(Info,1,#Info - 1)..[[},]] --1=2
end
...
function ExchangePropertyValue(item,file,FileTable,value_change_table,special_key_words,preceding_key_words
  ...
  save_section_to = ReturnStringFrom(save_section_to) 
  ...
  keep_section = ReturnStringFrom(keep_section) 
  ...
  add_named_section = ReturnStringFrom(add_named_section) 
  ...
  edit_section = ReturnStringFrom(edit_section) 
  ...
  elseif type(section_active) ~= "number" and type(section_active) ~= "string" and type(section_active) ~= "table" then
    ...
    elseif type(section_active) == "string" then
      local sa = math.tointeger(math.abs(tonumber(section_active)))
      if sa ~= nil then
        if sa >= 0 then
  ...
  local FoundNum = #GroupStartLine
          ...
          -- ShowSections(SectionsTable,"BFG:")        
  ...
  local IsMultipleSections = (#GroupStartLine > 1)
...
--END: the next 4 handlers sequence should be programmable !!!
  ...
  --===================================================
  -- NOW is the time to add the 'Using' info
  ...
  if Group_Found then
    ...
    while j <= (#val_change_table - 1) do
      ...
      if FoundNum > 0 and j == 1 then --just say on the first val_change_table
        local so = ""
        if IsSectionActive then
        ...
        if FoundNum > 1 then --when multiple sections found
          Report("","       >>>>> Found "..FoundNum.." valid candidate sections.")
          print("       >>>>> Found "..FoundNum.." valid candidate sections.")
          
          if IsMultipleSections and not (IsReplace or IsSectionActive) then
            Report("","KEY_WORDS located more than one section and REPLACE_TYPE/SECTION_ACTIVE are missing!","NOTICE")
            print(_zRED..">>> [NOTICE] KEY_WORDS located more than one section and REPLACE_TYPE/SECTION_ACTIVE are missing!".._zDEFAULT)
          end

          if IsSectionActive then
            Report("","       >>>>> using ACTIVE SECTIONS as requested: ["..string.sub(so,1,-2).."]")
            print("       >>>>> using ACTIVE SECTIONS as requested: ["..string.sub(so,1,-2).."]")
          
            if IsReplaceALL then
              Report("","SECTION_ACTIVE is overriding REPLACE_TYPE = 'ALL'","NOTICE")
              print(_zRED..">>> [NOTICE] SECTION_ACTIVE is overriding REPLACE_TYPE = 'ALL'".._zDEFAULT)
            end
            
          elseif IsReplaceALL then
            Report("","       >>>>> using ALL valid sections as requested")
            print("       >>>>> using ALL valid sections as requested")
          
          else
            Report("",[[       >>>>> 'Only FIRST section will be used']],"")
            print(_zRED..[[       >>>>> Only FIRST section will be used]].._zDEFAULT)
          end
        end
        ...
        if IsSectionActiveValuesNotUsed then
          Report("","SECTION_ACTIVE values were not used, out of range: ["..string.sub(so,1,-2).."]","NOTICE")
          print(_zRED..">>> [NOTICE] SECTION_ACTIVE values were not used, out of range: ["..string.sub(so,1,-2).."]".._zDEFAULT)
        end
        -- ***********************************************************************************
        -- print()
      end
      ...
      while k <= #GroupStartLine - 1 do
        ...
        elseif IsOnlyOnePreceding then
          pv("LastReplacementLine: "..LastReplacementLine)
          i = LastReplacementLine
...
function ProcessSECTION_ACTIVE(GSL,GEL,SKWL,SectionActive)
  ...
  for i=1,#SectionActive do
    if SectionActive[i] <= #GSL then
      table.insert(GSLA,GSL[SectionActive[i]])
      table.insert(GELA,GEL[SectionActive[i]])
      table.insert(SKWLA,SKWL[SectionActive[i]])
    else
      IsSkipped = true
    end
  end
  ...
  local function LocatePrecKeywordsInSection(TextFileTable,prec_key_words,index,StartLine,EndLine,level,IsQuit,groupId)
    ...
    for n = StartLine,EndLine do
      ...
      if string.find(line,[[">]],1,true) ~= nil then
        ...
        level = level + 1
        -- pv("level + 1 = "..level.." at "..n.." ["..line.."] ")
          
        if level >= currentLevel then
          local j = index
          local s = [["]]..string.upper(prec_key_words[j])..[["]]
          if string.find(line,[[PROPERTY NAME=]]..s,1,true) ~= nil or string.find(line,[[PROPERTY VALUE=]]..s,1,true) ~= nil then
            pv(string.rep(" ",100).."line "..n..": Found SOS: ["..prec_key_words[j].."] ")
            ...
            if j == #prec_key_words then
              ...
              pv(string.rep(" ",100).."line "..n..": Found LAST PK word ")
              ...
              pv(string.rep(" ",120).."*** OUT values: "..SectionStartLine[SectionNum].." - "..SectionEndLine[SectionNum].." ("..PrecKeyWordLine[SectionNum]..") ")

              if j > 1 then
                --on j == 1 we do NOT break so we can go down the file to the end
                pv("_BREAK_ on OUT values") --used
                break
              end
              ...
              if level == 0 or IsQuit then
                ...
                pv("_BREAK_ on level == 0 or IsQuit")
                break
              end
            end            
          end
      ...
      elseif string.find(line,[[Y>]],1,true) ~= nil then
        ...
        level = level - 1
        -- pv("level - 1 = "..level.." | currentLevel = "..currentLevel.." at "..n.." ")
        
        if level < currentLevel and index == 1 then
          pv("_BREAK_ on level < currentLevel and index == 1 at "..n.." ") --used
          break
        end
        ...
        if level <= 0 and n == EndLine then
          --we are done, end of file
          IsQuit = true
          pv("_BREAK_ on level == 0 and "..n.." == "..EndLine.." ")
          break
        end
      end
    end
            ...
            if All_PrecedingWords_Found then
              ...
              if #prec_key_words == 1 and prec_key_words[#prec_key_words] ~= "" then
                ...
                if All_PrecedingWords_Found then
                  ...
                  SectionsTable = AddSectionsIntoTable(GroupStartLine,GroupEndLine,SpecialKeyWordLine,"SKPK0",SectionsTable)
                  ShowSections(SectionsTable)        
                ...
                SectionsTable = AddSectionsIntoTable(GroupStartLine,GroupEndLine,SpecialKeyWordLine,"SKPK00",SectionsTable)
                ShowSections(SectionsTable)        
  ...
  for i=containerStartLine,#scriptTable do
    ...
    if scriptTable[i] ~= nil then    
      ...
      if string.sub(t,1,2) == "--" then
        --a comment at the start of the line, skip line
        skip = true
      elseif string.find(t,"--",1,true) ~= nil then
        --there is a comment somewhere in this line, remove it
